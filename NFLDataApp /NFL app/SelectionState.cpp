...
void SelectionState::initGui()
{
	...
	//Games dropdown list
	//adding all games played to the drop down list

	//Need to process my data files down even further to get the teams playing in each game
	...
	//Using postgreSQL I can call a query that returns all the games played in a season, and the plays within a match
	...
	//new way
	//trying to do the above dynamically:
	//defining SQL query
	std::string sql = "SELECT play_id, description FROM plays WHERE game_id = $1";

	//executing the above query for each game and storing the results
	pg::Results plays_BAL_LAC = db.execParams(sql, {game_id_BAL_LAC});
	pg::Results plays_CLE_BAL = db.execParams(sql, {game_id_CLE_BAL});
	// etc for other games

	//building the drop down:
	std::vector<std::string> options_BAL_LAC;
	for(auto& play : plays_BAL_LAC) {
		options_BAL_LAC.push_back(play["description"].as<std::string>());
	}

	//creating the drop down:
	this->plays_BALLAC = new gui::DropDownList(600, 115, 300, 50, font, options_BAL_LAC);
